...
library(dplyr)
library(ggplot2)
path_master='alex-badea_2023-12-05.xlsx'
data=read_xlsx(path_master )
data = as.data.frame(data)
#data = as.data.frame(t(na.omit(t(data))))
data$ALZDIS
datatemp=data%>%dplyr::select( SUBJECT_SEX, SUBJECT_AGE_SCREEN, AB40, AB42, PTAU181, APOE, NFL, TTAU)#subselect
#datatemp = as.data.frame((na.omit((data))))


datatemp$APOE[datatemp$APOE == "2/3" | datatemp$APOE == "3/3"] = "APOE3"
datatemp$APOE[datatemp$APOE == "2/4" | datatemp$APOE == "3/4" |  datatemp$APOE == "4/4"] = "APOE4"
datatemp$APOE[datatemp$APOE == "" ] = 0
# datatemp$APOE= as.numeric(datatemp$APOE)
unique(datatemp$APOE)


datatemp = datatemp[!is.na(datatemp$APOE),]



# plt = ggplot(datatemp, aes(x=SUBJECT_AGE_SCREEN, y= AB40 , color=as.factor(APOE)) ) +
#   geom_point() +
#   geom_smooth(method=lm, fullrange=TRUE, formula = y ~x   ) +
#   theme(axis.text=element_text(size=24), axis.title=element_text(size=28,face="bold"))+
#   xlab("Age")
#   # theme_linedraw() 
# 
# #   labs( title = paste0("" ) 
#  ggsave( paste0( "AB40", ".png") )
# 
#  plt = ggplot(datatemp, aes(x=SUBJECT_AGE_SCREEN, y= AB42 , color=as.factor(SUBJECT_SEX))) +
#    geom_point() +
#    geom_smooth(method=lm, fullrange=TRUE, formula = y ~x   ) +
#    theme_linedraw() 
#  #   labs( title = paste0("" ) 
#  ggsave( paste0( "AB42_sex", ".png") )
 Sex = datatemp $SUBJECT_SEX
 
 plt = ggplot(datatemp, aes(x=SUBJECT_AGE_SCREEN, y= datatemp$PTAU181 , color=as.factor(SUBJECT_SEX))) +
   geom_point(aes( size=10)) +
   geom_smooth(method=lm, fullrange=TRUE, formula = y ~x, aes(fill=factor(Sex)) ) +
   labs(color = "Sex (1M) (2F)", y = "PTAU181 in CSF")+
   theme_bw()+
     theme(axis.text=element_text(size=24), axis.title=element_text(size=28,face="bold"), 
   ...
   # theme_linedraw() 
 #   labs( title = paste0("" ) 
 ggsave( paste0( "PTAU181_sex", ".png") )


 plt = ggplot(datatemp, aes(x=SUBJECT_AGE_SCREEN, y= datatemp$PTAU181 , color=as.factor(APOE))) +
   geom_point(aes( size=10)) +
   geom_smooth(method=lm, fullrange=TRUE, formula = y ~x ,aes(fill=factor(APOE))  ) +
   labs(color = "APOE",  y = "PTAU181 in CSF")+
   theme_bw()+
   theme(axis.text=element_text(size=24), axis.title=element_text(size=28,face="bold"), 
         legend.title = element_text(size=18), legend.text = element_text(size=18))+
   # theme_bw()+
 ...
 # theme_linedraw() 
 #   labs( title = paste0("" ) 
 ggsave( paste0( "PTAU181", ".png") )
 
 
 
 
 
 plt = ggplot(datatemp, aes(x=SUBJECT_AGE_SCREEN, y= NFL , color=as.factor(SUBJECT_SEX))) +
   geom_point() +
   geom_smooth(method=lm, fullrange=TRUE, formula = y ~x  ) +
   theme_linedraw() 
 #   labs( title = paste0("" ) 
 ggsave( paste0( "NFL_sex", ".png") )

 
 plt = ggplot(datatemp, aes(x=SUBJECT_AGE_SCREEN, y= TTAU , color=as.factor(SUBJECT_SEX))) +
   geom_point() +
   geom_smooth(method=lm, fullrange=TRUE, formula = y ~x  ) +
   theme_linedraw() 
 #   labs( title = paste0("" ) 
 ggsave( paste0( "TTAU_sex", ".png") )
 
 

 plt = ggplot(datatemp, aes(x=SUBJECT_AGE_SCREEN, y= datatemp$AB42/datatemp$AB40 , color=as.factor(SUBJECT_SEX))) +
   geom_point() +
   geom_smooth(method=lm, fullrange=TRUE, formula = y ~x   ) +
   theme(axis.text=element_text(size=24), axis.title=element_text(size=28,face="bold"))+
   xlab("Age")
